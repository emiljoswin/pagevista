...
class RecentPage(Handler, Security, CachePosts):
	...
	def get_recent_posts(self):
		posts = {}
		postdb = self.loadRecentPostsDb(order = '-date', update = False)
		# postdb = PostsDB.all().order('-date').fetch(100)
		l = []
		for post in postdb:
	...
	def get(self):
		...
		if not username_ck or username_ck == 'None':
			self.pass_template_value_recent_page(login = "Login",signup = "Signup",posts = posts)
		else:
			username = username_ck
			username = self.return_username_if_valid_cookie(username_ck)
			self.pass_template_value_recent_page(logout = "Logout",username = username,posts = posts)
